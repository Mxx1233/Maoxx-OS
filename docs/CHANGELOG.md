...
- Phase 1D-E 审批决定在取得 request row lock 后使用 PostgreSQL `clock_timestamp()` 重新验证过期时间，避免锁等待跨越 `expires_at` 后沿用 transaction-start timestamp；监督 CLI 改用 GitHub check-runs newest-first API 顺序并验证唯一递减 check-run ID，确保较新的 queued/pending/in-progress/failed gate 无 `started_at` 时也不能被旧 success 覆盖。
